...
resfile="${1?Provide a path for the results file}"
...
for circ_id in ${!circuits[@]}; do
    ...
    if ! [ -f "$circdir/results.csv" ]; then
        echo "Results file for circuit $circuit does not exist"
        continue
    fi
    if [ $circ_id -eq 0 ]; then
        cat $circdir/results.csv > $resfile
    else
        sed -n '2,$ p' $circdir/results.csv >> $resfile
    fi
done

resfile_area="${resfile%.csv}_area_table.csv"
